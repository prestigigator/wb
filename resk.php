...
$rows = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM tc_sk WHERE cat_id = 'psk' ORDER BY `id` DESC"));//ORDER BY `id` DESC
...
mysqli_query($conn,"UPDATE `ranks_sk` SET `skoki_psk_o`='$sumawlasciwa' WHERE `ussid` = '$iduser0'")
				 or die ('Podczas wysyłania wystąpił błąd!'); 
...
$teee = mysqli_query($conn, "SELECT @rank:=@rank+1 AS `rank`, `ussid`, `skoki_psk_o`, `data_psk` FROM `ranks_sk` WHERE `skoki_psk_o` > 0 ORDER BY `skoki_psk_o` DESC, `data_psk` ASC");

while($rows5555 = mysqli_fetch_assoc($teee)){


	$ttt = "UPDATE `ranks_sk` SET `joined_psk`='".$rows5555['rank']."' WHERE `ussid` = '".$rows5555['ussid']."'";
...
if(isset($_POST['generalka'])){
	$tt1 = "UPDATE `ranks_sk` SET `skoki_psk`=`skoki_psk`+`skoki_psk_o`";
	mysqli_query($conn, $tt1);
	echo $msg = "OGeneralka chyba wczytana";
}else{
	//
}
...
if(isset($_POST['zerowanie'])){
	$tt1 = "UPDATE `ranks_sk` SET `skoki_psk_o`=0, `joined_psk`=0";
	mysqli_query($conn, $tt1);
	echo $msg = "wyzerowane";
}else{
	//
